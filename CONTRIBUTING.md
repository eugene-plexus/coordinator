# Contributing to Eugene Plexus `coordinator`

Thanks for your interest. This service implements the `coordinator` OpenAPI contract from [`eugene-plexus/specs`](https://github.com/eugene-plexus/specs) — please read this before opening a PR.
...
- **Engine work** — the cross-component pipeline-execution engine, stage sequencing, peer delegation.
...
```bash
git clone https://github.com/eugene-plexus/coordinator
cd coordinator
python -m venv .venv
...
File issues at <https://github.com/eugene-plexus/coordinator/issues>. Useful issues include:

- Concrete pipeline-lifecycle failures with reproduction steps.
- Spec-vs-impl divergence (the impl drifted from `coordinator.yaml`).
